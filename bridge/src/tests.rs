...
mod integration {
    ...
    async fn test_bridge_startup_with_inprocess_node() {
        ...
        let kaspa_api = KaspaApi::new(rpc_address.clone(), None, shutdown_rx.clone(), None).await.unwrap();
...
mod comprehensive_tests {
    ...
    fn test_client_handler_extranonce_wrapping_edge() {
        ...
        let share_handler = Arc::new(ShareHandler::new("test-instance".to_string()));
        let handler = ClientHandler::new(share_handler, 8192.0, std::collections::HashMap::new(), 2, "test-instance".to_string());
